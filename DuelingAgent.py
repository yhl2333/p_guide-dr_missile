...
'''
Dueling DQN改进神经网络的结构，将状态动作价值函数分为两部分：
...
class Approximator(torch.nn.Module):
    def __init__(self, state_dim=1, action_dim=1, hidden_dim=20):
        ...
        self.fc2_adv = torch.nn.Linear(self.hidden_dim, self.action_dim)
        self.fc2_val = torch.nn.Linear(self.hidden_dim, 1)

    def forward(self, x):
        x = F.relu(self.fc1(x))
        adv = self.fc2_adv(x)
        val = self.fc2_val(x).expand(x.size(0), self.action_dim)
...
class Agent(object):
    ...
    def _optimize_model(self):
        ...
        # 取实际使用的动作对应的动作价值
        # gather(dim, index) 输出形如index的tensor
        state_action_values = self.policy_net(state_batch).gather(1, action_batch)

        next_state_values = torch.zeros(BATCH_SIZE, device=device)

        # double DQN更新误差为 Y_t = R_{t+1} + \gamma*Q(S_{t+1}, argmax_a Q(S_{t+1},a;\theta_t);\theta_t^-)
        policy_max_action = self.policy_net(non_final_next_states).argmax(1).unsqueeze(1)
        next_state_values[non_final_mask] = self.target_net(non_final_next_states).gather(1, policy_max_action).squeeze(1)
    ...
    def learning(self):
        ...
        for i_episode in range(NUM_EPISODES):
            ...
            # 每一万轮保存模型参数
            if (i_episode + 2) % 1000 == 0:
                self.save_model('model/airComDuel' + str(i_episode))
